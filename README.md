...
> 面向消费电子出海的 **AI 市场分析与业务辅助平台**（v1.0.3 稳定版）。核心能力：**🤖 AI Agent 一句话全流程**（"蓝牙耳机去德国卖" → 自动完成 市场分析 → 报告 → 客户线索 → 定制开发信，SSE 实时进度）；另有五个业务模块：**市场分析**（产品+国家 → 结构化市场报告 + 多国横向对比 + 历史记录）、**贸易数据**（HS 编码 → 出口数据 + 趋势图 + AI 解读 + 竞争力指标 + 定价建议）、**外贸业务**（英文开发信 / 跟进邮件 / 产品介绍 / AI 模拟客户）、**跨境电商**（商品采集 / 评论分析 / 竞品对比 / Listing / 财报画像 / eBay + 速卖通商品分析）、**客户线索**（Tavily 检索 + 防幻觉硬约束 + 定向开发信 + 销售漏斗），以及**我的订阅**（持续监控）、**管理面板**（安全拦截记录 / 服务器 Key 状态）。
...
## v1.0.2 更新日志（2026-08-15）— 基于 v1.0/v1.0.1 的重大修复合集

> v1.0.2 = v1.0.1 数据准确性专项 + 数据层架构收口 + 业务闭环 + 多轮安全/健壮性加固，
> 是当前主线的完整版本。数据层让每个数字**可信可追溯**，业务层让分析**能落地成行动**。
...
**🤖 AI 质量注入**
- 数据置信度总览（✅可信/⚠️存疑/❌拒绝）+ 引用可信基础纪律（结论必须说明数据来源）
- 市场总进口作为市场规模推算底数（AI 不再把出口额当市场规模或编造进口额）
- REJECTED 数字直接不入提示词（程序判定质量，AI 不解读不可信数据）

### 7. 安全与健壮性（多轮排查 36+20+13 处）

- **安全**：XSS（chart-meta 纯 DOM）、Docker 密钥泄漏（.dockerignore + 非 root）、
  SEARCH_BASE_URL 校验（带 Key 外发防线）、422 错误可读化、SSRF DNS rebinding IP 钉扎
- **稳定性**：get_latest_year 429 熔断 + UnboundLocalError、Word COM 45s 超时、
  同国查询明确报错（市场分析放开/贸易页拦截）、LLM 缓存 key 规范化（防烧双倍 token）
- **可观测性**：静默失败排查（value=0 伪装修复 + 8 处日志）、AI HS 解析失败留痕、
  计算审计 `_audit`、数据新鲜度 🟢🟡🟠🔴 徽章

**测试**：129 项 pytest 全部通过（脏数据 8 case、DataGate、血缘、定价、熔断、同国守卫、置信度总览等回归）。

---

## v1.0.3 更新日志（2026-08-31）— 数据准确性修复

> v1.0.3 修复了竞争力指标（TC + 市场份额）与定价建议在德国/欧盟类市场的**静默失效**，
> 以及一系列数据准确性与安全防线漏洞。核心：让每个数字都能算出来、算得对、且口径透明。

### 🔴 竞争力指标 + 定价建议恢复（德国类市场此前 100% 失效）

**根因**：查询「目标市场总进口」（市场份额分母）时用 `partnerCode=0`（全球口径），UN Comtrade 会按 `partner2Code`（原产国）拆行——德国 2024 年进口 HS 8518 被拆成 106 个原产国，撑爆 500 条记录上限，被完整性校验拒绝后异常被静默吞掉。全项目此前未处理过 `partner2Code` 维度。

| 修复 | 内容 | 实测 |
| --- | --- | --- |
| API 侧要合计行 | `fetch_year` 请求加 `partner2Code=0` | 500 行 → 18 行，总额 38.73 亿美元（正确） |
| 去重键补维度 | 去重键加 `partner2Code`，堵住"106 原产国折叠成 1 行"的潜伏雷（差 1900 倍） | 中国→德国主查询加参数前后均 1 行同值，无副作用 |

**修复后实测**（蓝牙耳机 → 德国，UN Comtrade 正式接口）：TC = **0.724**、中国占德国市场进口份额 = **15.33%**、定价建议区间 **23.07–57.58 美元/公斤**，全部恢复且带数据血缘（quality=valid）。

### 🔴 竞争对手份额虚高修复

任一出口国数据获取失败（如英国撞 500 上限）时，原实现把缺失份额**等比摊给剩余国家**（中国 34% → 52%）。修复：有失败国时所有国家份额一律置 null，显示"数据缺失"而非虚高占比；竞争力矩阵对失败国不再写缓存，瞬时失败不固化。

### 🟡 数据血缘键名错位

`get_cache_meta` 返回 `validation_reason`，但有两处按 `reason` 取值——日志 REJECTED 原因恒空、定价拒绝分支 KeyError 被吞成通用文案。已统一键名，拒绝原因透出恢复。

### 🟡 SSRF 防线绕过（安全类）

`config.py` 与 `collectors.py` 内网地址判定对 IPv6 只查环回/链路本地，漏查 `is_private`——IPv4-mapped 地址（`::ffff:192.168.1.1`）被当公网放行。已补：IPv6 先解包 IPv4-mapped 再按 IPv4 规则判定。

### 🟡 品牌份额口径不透明（数据可信类）

报告"热门品牌"曾把"2021 年中国市场出货量份额"用于德国报告。修复：网页报告与 Word 报告的品牌表均补「份额（口径）」列，口径与份额同列标注；竞争格局防幻觉校验改为按口径分组各自求和（不再把不同口径份额相加误判）。

**测试**：139 项 pytest 全部通过（原 129 + 新增回归 10 项 `tests/test_regression_20260831.py`）。
...
| ✅ v1.0.2 | 重大修复合集 | 数据准确性（UN Comtrade 聚合）+ 数据层收口（血缘/DataGate/四态质量）+ 业务闭环（销售漏斗/定价建议/市场订阅）+ 安全加固（XSS/密钥/SSRF/熔断）——129 项测试 |
| ✅ v1.0.3 | 数据准确性修复 | 竞争力指标/定价恢复（partner2Code 拆行）+ 份额虚高修复 + 血缘键名 + IPv4-mapped SSRF + 品牌份额口径透明——139 项测试 |

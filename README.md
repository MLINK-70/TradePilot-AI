...
| 502 | DeepSeek API 调用失败（超时 / 余额不足 / Key 无效） |
...
├── llm.py           # DeepSeek 调用层：请求、JSON 解析、错误处理
...
| 第二版 | 外贸业务 Business | 开发信 / 跟进邮件 / 产品介绍 / 客户画像 |
| 第三版 | 跨境电商 E-commerce | Amazon 评论分析 → 用户痛点总结 |
| 第四版 | 贸易数据 Trade | HS 编码 / 产品名 → 出口数据 + 趋势图 |
...
本项目的差异化定位：**把 AI 能力应用到国际贸易场景**。既有消费电子行业洞察（用户画像 / 风险分析 / 市场趋势），也有工程实践（FastAPI 后端分层、提示词工程、前端安全渲染），适合作为求职作品持续迭代。
